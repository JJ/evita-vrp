 ...
 *  
 *   Copyright 2008 Universidad de Granada
 *  
...
/**
 *
 * @author Pablo Garcia-Sanchez (ferguson) (pgarcia@geneura.ugr.es)
 */
...
public class SimpleCW {
    ...
    private List<Shop> shops4Today;
    private ITPdata data;
    public ArrayList<Route> bestSolution;
	...
	public SimpleCW( List<Shop> shops4Today, ITPdata data) {
		...
		num_rutas = shops4Today.size();
                this.shops4Today=shops4Today;
                this.data = data;
                bestSolution = new ArrayList<Route>();
		depot  = data.shopList.get(0);
    ...
    private boolean combinacion_posible(int [] r_mejores){
        ...
        if (tiempo > this.data.maximumWorkTime || carga > this.data.vehicleCapacity)
            p=false;
    ...
    private void convertir_a_rutas(){
    	 ...
    	 for (int i=1; i<posicionLista.size();i++){
    		 ...
    		 if (siguiente == 0){
    			...
    			R.distanceTravelled += this.data.distanceTable[actual][siguiente];
    ...
    private double coste_trayecto(int actual,int siguiente){
    	return this.data.distanceTable[actual][siguiente]*this.data.costPerKm;
    }
    ...
    private double tiempo_trayecto(int actual,int siguiente){
    	...
    	double t = this.data.distanceTable[actual][siguiente]/this.data.speed;
    	
    	if (siguiente !=0)
    		t += this.data.downloadTime;
